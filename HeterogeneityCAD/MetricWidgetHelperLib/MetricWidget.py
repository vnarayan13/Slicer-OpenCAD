...
class MetricWidget(qt.QCheckBox):
  def __init__(self, parent=None):
    super(MetricWidget, self).__init__(parent)
        
  def Setup(self, featureName, description, check=True):
    self.setText(featureName)
    self.checked = check

    self.setContextMenuPolicy(3)
    self.widgetMenu = self.ContextMenu(self, description) 
    self.customContextMenuRequested.connect(lambda point: self.widgetMenu.connectAndMap(point))
